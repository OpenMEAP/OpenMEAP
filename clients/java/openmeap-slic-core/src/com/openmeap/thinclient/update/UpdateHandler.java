...
/**
 * Handles all the business logic for performing an update. Pulled into SLIC
 * core for re-use between Android and RIM OS
 * 
 * @author schang
 */
public class UpdateHandler {
	...
	public UpdateHandler(OmMainActivity activity, SLICConfig config,
	...
	public ConnectionOpenResponse makeConnectionOpenRequest(
			ConnectionOpenRequest request) throws WebServiceException {
		// phone home
		ApplicationManagementService client = AppMgmtClientFactory
				.newDefault(config.getAppMgmtServiceUrl());
	...
	public ConnectionOpenRequest getConnectionOpenRequest() {
		...
		app.setHashValue(config.getArchiveHash() != null ? config
				.getArchiveHash() : "");

		ApplicationInstallation dev = request.getApplication()
				.getInstallation();
	...
	/**
	 * Interface so that the HTML5 application can provide a call-back for
	 * update status.
	 * 
	 * @author schang
	 */
	public interface StatusChangeHandler {
		/**
		 * Gets called each percent of completion throughout the duration of the
		 * download
		 * 
		 * @param header
	 ...
	 * @param updateHeader
	 */
	public void handleUpdate(final UpdateHeader updateHeader,
			final StatusChangeHandler eventHandler) {
				...
				public void run() {
					try {
						...
						if (e instanceof UpdateException) {
							...
							ue = new UpdateException(UpdateResult.UNDEFINED,
									e.getMessage(), e);
						}
						config.setLastUpdateResult(ue.getUpdateResult()
								.toString());
						update.setError(ue);
	...
	private void _handleUpdate(UpdateStatus update,
			StatusChangeHandler eventHandler) throws UpdateException {
		...
		if (!hasTimedOut
				...
				&& lastUpdateResult.compareTo(UpdateResult.PENDING.toString()) == 0) {
			return;
		} else {
		...
		if (!deviceHasEnoughSpace(update).booleanValue()) {
			// TODO: whether this is a deal breaker or not should be
			// configurable. client should have the ability to override default
			// behavior. default behavior should be informative
			throw new UpdateException(UpdateResult.OUT_OF_SPACE,
					"Need more space to install than is available");
		}
		...
		try {
			...
			// configurable. client should have the ability to override default
			// behavior. default behavior should be informative
			throw new UpdateException(UpdateResult.IO_EXCEPTION,
					"An issue occurred downloading the archive", ioe);
		}

		if (!archiveIsValid(update).booleanValue()) {
			throw new UpdateException(UpdateResult.HASH_MISMATCH,
					"The import archive integrity check failed");
		}
		...
		try {
			...
			throw new UpdateException(UpdateResult.IO_EXCEPTION,
					"Could not delete import archive", lse);
		}
		...
		try {
			...
			throw new UpdateException(UpdateResult.IO_EXCEPTION,
					"Could not reset storage", lse);
		}

		config.setLastUpdateResult(UpdateResult.SUCCESS.toString());

		config.setApplicationVersion(update.getUpdateHeader()
				.getVersionIdentifier());
		config.setArchiveHash(update.getUpdateHeader().getHash().getValue());

		String newPrefix = storage.getStorageRoot()
				+ update.getUpdateHeader().getHash().getValue();
	...
	public Boolean deviceHasEnoughSpace(UpdateStatus update)
		...
		try {
			...
			throw new UpdateException(UpdateResult.IO_EXCEPTION,
					"Could not determine the number of bytes available", e);
		}
		return new Boolean(avail.longValue() > update.getUpdateHeader()
				.getInstallNeeds().longValue());
	}
	 ...
	 * @throws IOException
	 */
	public Boolean downloadToArchive(UpdateStatus update,
			StatusChangeHandler eventHandler) throws UpdateException {
		...
		try {
			updateRequestResponse = requester.get(update.getUpdateHeader()
					.getUpdateUrl());
		} catch (HttpRequestException e) {
			throw new UpdateException(UpdateResult.IO_EXCEPTION,
					"An issue occurred fetching the update archive", e);
		}
		if (updateRequestResponse.getStatusCode() != 200)
			throw new UpdateException(UpdateResult.RESPONSE_STATUS_CODE,
					"Status was " + updateRequestResponse.getStatusCode()
							+ ", expecting 200");
		try {
			...
			while (count != (-1)) {
				...
				if (eventHandler != null
						&& lastContentDownloaded + percent < contentDownloaded) {
				...
				synchronized (interruptLock) {
					if (interrupt.booleanValue()) {
						clearInterruptFlag();
						throw new UpdateException(UpdateResult.INTERRUPTED,
								"Download of archive was interrupted");
					}
				}
			}
	...
	public Boolean archiveIsValid(UpdateStatus update) throws UpdateException {
		try {
			...
			try {
				...
				String hashValue = Utils.hashInputStream(update
						.getUpdateHeader().getHash().getAlgorithm().value(),
						fis);
				if (!hashValue.equals(update.getUpdateHeader().getHash()
						.getValue())) {
			...
			throw new UpdateException(UpdateResult.UNDEFINED,
					"The archive failed validation", e);
		}
	}
	...
	public void installArchive(UpdateStatus update) throws UpdateException {
		try {
			...
			throw new UpdateException(UpdateResult.IO_EXCEPTION,
					"The archive failed to install", e);
		}
	}
	...
	private boolean hasUpdatePendingTimedOut() {
		...
		if (lastAttempt != null) {
			return currentTime.longValue() > lastAttempt.longValue()
					+ (pendingTimeout.intValue() * 1000);
		}
		return true;
	}
	...
	public void initialize(OmWebView webView) {
		...
		Boolean shouldPerformUpdateCheck = activity.getConfig()
		...
		if (shouldPerformUpdateCheck != null
				&& shouldPerformUpdateCheck.equals(Boolean.TRUE)) {
			new Thread(new UpdateCheck(webView)).start();
		}
	}

	private void _revertToOriginal(UpdateStatus update,
			StatusChangeHandler eventHandler) throws UpdateException {
		config.setApplicationVersion(update.getUpdateHeader()
				.getVersionIdentifier());
		config.setArchiveHash(update.getUpdateHeader().getHash().getValue());
		...
		public void run() {
			...
			if (update != null && update.getType() == UpdateType.IMMEDIATE) {
				try {
					...
					err = new WebServiceException(
							WebServiceException.TypeEnum.CLIENT_UPDATE,
							e.getMessage(), e);
					if (activity.getReadyForUpdateCheck()) {
						try {
							webView.setUpdateHeader(null, err,
									storage.getBytesFree());
